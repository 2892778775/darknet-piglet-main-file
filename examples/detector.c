...
{
    list *options = read_data_cfg(datacfg);//将数据及配置转化为链表
    ...
    printf("%s\n", base);
    float avg_loss = -1;
    network **nets = calloc(ngpus, sizeof(network));//为每个gpu分配一个network，所以在使用多gpu时会多次加载网络模型，且每个gpu占用内存相同
    ...
    while(get_current_batch(net) < net->max_batches){
        if(l.random && count++%10 == 0){//网络文件中的最后random参数
            printf("Resizing\n");
            int dim = (rand() % 10 + 10) * 32;//320-608之间多尺度训练，模型的泛化能力更强
    ...
    while(1)
    {
		...
		struct dirent *imagename;   //readdir return 
		    ...
		    detection *dets = get_network_boxes(net, im.w, im.h, thresh, hier_thresh, 0, 1, &nboxes);
         	    ...
         	    draw_detections_piglet(imagename->d_name, odir, im, dets, nboxes, thresh, names, alphabet, l.classes);
            	    free_detections(dets, nboxes);
            	    char imagesdir[512];
	    ...
	    draw_detections_piglet(input, odir, im, dets, nboxes, thresh, names, alphabet, l.classes);
        free_detections(dets, nboxes);
        if(outfile){
...
void run_detector(int argc, char **argv)
{
    ...
    float thresh = find_float_arg(argc, argv, "-thresh", .5);//输出置信度默认阈值
    ...
    char *filename = find_char_arg(argc, argv, "-input", 0);//timen:单张测试
    char *idir = find_char_arg(argc, argv, "-idir", 0);//批量测试参数设置 输入文件夹
    char *odir = find_char_arg(argc, argv, "-odir", 0);//输出文件夹
